...
#
# Note: this script must be run with the repo root as the working directory
# (e.g. `.\scripts\deploy\export_emulators.ps1` from root) - firebase emulators:export
# writes its firebase-export-* temp folder relative to the invoking shell's CWD,
# not this script's own location, so $projectDir is derived from CWD, not $PSScriptRoot.
