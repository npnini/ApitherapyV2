...
function cleanupOrphanedRootFolders() {
  try {
    ...
    rootFiles.forEach(file => {
      if (file.startsWith('firebase-export-')) {
        const fullPath = `./${file}`;
        try {
...
async function saveEmulatorData(reason = 'scheduled') {
  ...
  while (attempt < maxRetries && !success) {
    ...
    try {
      ...
      // We explicitly pass the --project parameter to avoid failures when the active workspace project is switched
      ...
      const exportsFiles = fs.readdirSync('./.firebase-exports');
      exportsFiles.forEach(file => {
