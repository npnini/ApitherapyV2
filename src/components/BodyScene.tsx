...
const BodyScene: React.FC<BodySceneProps> = ({ 
  ...
  const INITIAL_DIST = 5.2;
  useFrame((state) => {
    if (!controlsRef.current || !onZoomChange) return;
    const currentDist = state.camera.position.distanceTo(controlsRef.current.target);
    const rawScale = INITIAL_DIST / currentDist;
    // Map the 3D zoom to a 2D viewport expansion.
    // We only start expanding after some zoom in, and cap at 2.5x.
    const targetScale = Math.max(1, Math.min(2.5, rawScale));
    
    if (Math.abs(lastReportedScale.current - targetScale) > 0.05) {
      lastReportedScale.current = targetScale;
