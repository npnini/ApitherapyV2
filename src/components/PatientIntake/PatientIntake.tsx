...
const PatientIntake: React.FC<PatientIntakeProps> = ({ patient, onSave, onUpdate, onBack, saveStatus, errorMessage }) => {
    ...
    const renderStep = () => {
        switch (currentStep) {
            case 1:
                return <PersonalDetails patientData={patientData} onDataChange={handleDataChange} showErrors={hasAttemptedSubmit} />;
            case 2:
                ...
                />;
            default:
                return <div><T>Unknown Step</T></div>;
        }
    };
            ...
            <div className="relative bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-full overflow-hidden">
                <div className={`${styles.modalHeader} bg-blue-600 text-white`}>
                    <h3 className="text-lg font-semibold">
                        {patient.id ? <><T>Patient Details</T> - {patientData.fullName}</> : <T>Patient Intake Process</T>}
                    </h3>
                    <div className={styles.headerRight}>
                        <button type="button" onClick={onBack} className="text-gray-200 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center">
                            ...
                            <span className="sr-only"><T>Close modal</T></span>
                        </button>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between p-4 md:p-5 border-t border-gray-200 rounded-b">
                    ...
                    <div className="flex items-center">
                        {saveStatus === 'saving' && <div className="text-sm text-gray-500"><T>Saving</T>...</div>}
                        {saveStatus === 'success' && <div className="text-sm text-green-500"><T>Saved successfully!</T></div>}
                        {saveStatus === 'error' && errorMessage && <div className="text-sm text-red-500"><T>{errorMessage}</T></div>}
